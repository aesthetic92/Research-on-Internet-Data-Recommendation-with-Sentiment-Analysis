...
def sentence_sentiment_score(dataset):    # each item
    cuted_review = []
    for cell in dataset:
...
def store_sentiment_dictionary_score(review_set, storepath):
    sentiment_score = all_review_sentiment_score(sentence_sentiment_score(review_set))     #each goods each items
    pos = 0
    neg = 0
    f = open(storepath,'w')
    value = []
    for i in sentiment_score:   #each goods each items
        pos = pos + i[0]
        neg = neg + i[1]
