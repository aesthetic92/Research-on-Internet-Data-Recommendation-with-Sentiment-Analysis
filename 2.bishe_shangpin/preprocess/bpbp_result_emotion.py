...
def bp():
    ...
    for row in rows:
        if precess(row) < INF:
            arr1.append(math.log(int(row[1])+1, index))
            ...
            out_result.append(math.log(int(row[7])+1, index))
            score.append(math.log(abs(emotion[int(row[0])])+1, index))
            # score.append(emotion[int(row[0])])
            old_result.append(int(row[7]))

    samplein = np.mat([arr1,arr2,arr3,arr4,arr5,arr6, score]) #6*100
    ...
    for i in xrange(len(networkout2[0])):
        new_result.append(2 ** networkout2[0][i])
