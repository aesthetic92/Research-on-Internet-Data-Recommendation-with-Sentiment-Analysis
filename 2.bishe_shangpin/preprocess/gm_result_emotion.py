...
def gm():
    ...
    for row in rows:
        if precess(row) < INF:
            history_data = []
            old_result.append(int(row[7])+1)
            for i in xrange(len(row)-2):
                history_data.append(math.log(int(row[i+1])+1, index))
            n = len(history_data)
            ...
            result = (X0[0] - u/a)*(1-math.exp(a))*math.exp(-a*(n))*getEmotion(row[0])
            new_result.append(2 ** result)
    return  new_result
