...
def bp():
    ...
    for row in rows:
        ...
        score_text.append(math.log(abs(emotion[str(row[0])]*getEcoEmotion())+1, index))
        # score_text.append(emotion[str(row[0])]*getEcoEmotion())
        old_result.append(int(row[1]))
        out_result.append(math.log(int(row[1])+1, index))

    samplein = np.mat([arr2,arr3,arr4,arr5,arr6,arr7,score_text]) #8*100
